...
### 用户权限管理（接口已定义，待实现）
- **多用户支持** — 管理员/编辑者/访客三种角色（数据层 stub，返回空列表）
- **权限控制** — admin 全部权限，editor 上传/删除，viewer 只读（待实现）
- **用户登录** — 支持用户名密码登录，角色权限验证（待实现）
- **用户管理** — 管理员可添加/删除用户，修改用户角色（待实现）

### 搜索增强（接口已定义，待实现）
- **搜索历史** — 自动保存最近 20 条搜索记录，一键复用（Web 端实现）
- **实时搜索建议** — 输入时显示匹配的媒体文件（Web 端实现）
- **高级搜索** — 按类型/文件夹/大小范围筛选（数据层 stub，返回空列表）
- **全局搜索** — 跨设备搜索媒体文件（数据层 stub，返回空列表）

### 媒体信息增强（接口已定义，待实现）
- **媒体元数据** — 显示标题/艺术家/专辑/时长/分辨率/码率/编码/格式（数据层 stub，返回 null）
- **海报墙展示** — 显示媒体海报/封面（待实现）
- **标签系统** — 支持媒体标签分类（待实现）
- **字幕支持** — 显示关联的字幕文件（待实现）
